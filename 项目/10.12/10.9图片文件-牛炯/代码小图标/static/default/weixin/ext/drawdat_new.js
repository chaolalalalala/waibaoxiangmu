...
$(function () {
    function getOption_type(deal_product) {
        ...
        $.ajax({
            type: 'post',
            url: ROOT + '/weixin/index/marketlist',
    ...
    alert(ROOT+'/weixin/index/marketlist');
...
// //关闭广告
// $(".ad-close").on("coptionslick",function(e){
// 	$("#ad").hide();
// 	e.stopPropagation();
// });
//首页tab切换
        ...
        success: function (response) {
            ...
            if (response.status == 1) {
                ...
                $.each(response.list, function (key, value) {
                    ...
                    $("#now_list").append('<tr class="rec-tr" onclick="get_history(\'' + value.id + '\',this)" id="' + value.id + '"><td class="rec-td fcgray3">' + value.trade_time + '</td><td class="rec-td">' + value.option_key + '</td><td class="rec-td">' + value.trade_direction + '</td><td class="rec-td fcgray3">' + value.trade_point + '</td><td class="rec-td fcgray3 now_price">' + value.trade_point + '</td><td class="rec-td fcgray3">' + value.is_settle + '</td></tr>');
                    $(".come-txt").text("");
                });
            } else {
                if (num == 1) {
