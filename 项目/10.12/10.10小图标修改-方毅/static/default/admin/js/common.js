...
;$(function () {
	...
	$('.ajax-get').click(function () {
		...
		var prompt_title = $(this).attr('prompt-title') ? $(this).attr('prompt-title') : '';
		var parm = '';
		if(prompt_title) {
			var target = $(this).attr('href') ? $(this).attr('href') : (target = $(this).attr('url') ? target = $(this).attr('url') : '');
			layer.prompt({title : prompt_title, formType : 2}, function(text, index) {
				layer.close(index);
				if (target) {
					$.get(target, {param : text},
						function (data) {
							if (data.status == 1) {
								if (data.url) {
